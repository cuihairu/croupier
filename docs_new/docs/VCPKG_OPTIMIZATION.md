...
- name: Cache vcpkg
  ...
  with:
    path: |
      {% raw %}${{ github.workspace }}/vcpkg/installed
      ${{ github.workspace }}/vcpkg/buildtrees{% endraw %}
    key: {% raw %}${{ runner.os }}-vcpkg-${{ hashFiles('vcpkg.json') }}-${{ matrix.triplet }}{% endraw %}
    restore-keys: |
      {% raw %}${{ runner.os }}-vcpkg-${{ hashFiles('vcpkg.json') }}-{% endraw %}
      {% raw %}${{ runner.os }}-vcpkg-{% endraw %}
```
