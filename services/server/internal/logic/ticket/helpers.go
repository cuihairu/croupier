...
func buildTicketDTO(ticket *model.Ticket) types.Ticket {
	return types.Ticket{
		Id:        int64(ticket.ID),
		Title:     ticket.Title,
...
func buildCommentsDTO(comments []model.TicketComment) []types.Comment {
	dto := make([]types.Comment, 0, len(comments))
	for _, c := range comments {
		dto = append(dto, types.Comment{
			Id:        int64(c.ID),
			Content:   c.Content,
			Author:    c.Author,
...
func sanitizeTicketFields(req *types.TicketCreateRequest) (*model.Ticket, error) {
