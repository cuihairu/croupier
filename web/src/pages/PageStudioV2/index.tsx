/**
 * PageStudio vNext - 语义化页面编辑器
 *
 * 使用 vNext 强类型 PageSpecV2，不使用 Formily schema。
...
function pageTypeLabel(type: PageTypeV2): string {
  switch (type) {
...
// ---------------------------------------------------------------------------
// PageStudio vNext Component
// ---------------------------------------------------------------------------

export default function PageStudioV2() {
  const { message } = App.useApp();
  const [drafts, setDrafts] = useState<PageSpecDraftSummary[]>([]);
  const [loading, setLoading] = useState(false);
  const [selectedDraft, setSelectedDraft] = useState<PageSpecV2 | null>(null);
  ...
  const loadDraftDetail = useCallback(async (pageKey: string) => {
    try {
      ...
      setSelectedDraft(draft as unknown as PageSpecV2);
  ...
  const columns: ProColumns<PageSpecDraftSummary>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Tag color="blue">{pageTypeLabel(record.type as PageTypeV2)}</Tag>
        </Space>
      ),
    },
    {
