...
import (
    sup "github.com/cuihairu/croupier/internal/repo/gorm/support"
...
func (s *Server) addSupportRoutes(r *gin.Engine) {
	...
	r.GET("/api/support/tickets", func(c *gin.Context) {
		...
		if err := db.Count(&total).Error; err != nil {
			s.respondError(c, 500, "internal_error", "count failed")
			return
		}
		var arr []sup.Ticket
		if err := db.Order("updated_at DESC").Limit(size).Offset((page - 1) * size).Find(&arr).Error; err != nil {
			s.respondError(c, 500, "internal_error", "list failed")
			return
		}
	...
	r.POST("/api/support/tickets", func(c *gin.Context) {
		...
		if err := c.BindJSON(&in); err != nil || strings.TrimSpace(in.Title) == "" {
		...
		if err := s.gdb.Create(t).Error; err != nil {
			s.respondError(c, 500, "internal_error", "create failed")
			return
		}
		if s.audit != nil {
			_ = s.audit.Log("support.ticket_create", user, strconv.FormatUint(uint64(t.ID), 10), map[string]string{"ip": c.ClientIP(), "title": t.Title})
		}
	...
	r.GET("/api/support/tickets/:id", func(c *gin.Context) {
		...
		var t sup.Ticket
		if err := s.gdb.First(&t, c.Param("id")).Error; err != nil {
	...
	r.PUT("/api/support/tickets/:id", func(c *gin.Context) {
		...
		if err := s.gdb.Save(&t).Error; err != nil {
			s.respondError(c, 500, "internal_error", "update failed")
			return
		}
		if s.audit != nil {
			_ = s.audit.Log("support.ticket_update", user, strconv.FormatUint(uint64(t.ID), 10), map[string]string{"ip": c.ClientIP(), "status": t.Status, "assignee": t.Assignee})
		}
		c.Status(204)
	})
	r.DELETE("/api/support/tickets/:id", func(c *gin.Context) {
		...
		if err := s.gdb.Delete(&sup.Ticket{}, c.Param("id")).Error; err != nil {
			s.respondError(c, 500, "internal_error", "delete failed")
			return
		}
		if s.audit != nil {
			_ = s.audit.Log("support.ticket_delete", user, c.Param("id"), map[string]string{"ip": c.ClientIP()})
		}
		c.Status(204)
	})
	...
	r.GET("/api/support/tickets/:id/comments", func(c *gin.Context) {
		...
		if err := s.gdb.Where("ticket_id = ?", c.Param("id")).Order("created_at ASC").Find(&arr).Error; err != nil {
			s.respondError(c, 500, "internal_error", "list failed")
			return
		}
	...
	r.POST("/api/support/tickets/:id/comments", func(c *gin.Context) {
		...
		if err := c.BindJSON(&in); err != nil || strings.TrimSpace(in.Content) == "" {
			s.respondError(c, 400, "bad_request", "invalid payload")
			return
		}
		cm := &sup.TicketComment{TicketID: parseUint(c.Param("id")), Author: user, Content: in.Content, Attach: in.Attach}
		if err := s.gdb.Create(cm).Error; err != nil {
			s.respondError(c, 500, "internal_error", "create failed")
			return
		}
		if s.audit != nil {
			_ = s.audit.Log("support.ticket_comment", user, c.Param("id"), map[string]string{"ip": c.ClientIP()})
		}
	...
	r.POST("/api/support/tickets/:id/transition", func(c *gin.Context) {
		...
		if err := c.BindJSON(&in); err != nil || strings.TrimSpace(in.Status) == "" {
			s.respondError(c, 400, "bad_request", "invalid payload")
			return
		}
		var t sup.Ticket
		...
		if s.audit != nil {
			_ = s.audit.Log("support.ticket_transition", user, strconv.FormatUint(uint64(t.ID), 10), map[string]string{"ip": c.ClientIP(), "from": from, "to": t.Status})
		}
		c.Status(204)
	})
	...
	r.GET("/api/support/faq", func(c *gin.Context) {
		...
		if q != "" {
			db = db.Where("question LIKE ? OR answer LIKE ?", "%"+q+"%", "%"+q+"%")
		}
		if category != "" {
			db = db.Where("category = ?", category)
		}
		if visible != "" {
			if visible == "true" || visible == "1" {
				...
				db = db.Where("visible = ?", false)
			}
		}
		var arr []sup.FAQ
		if err := db.Order("sort DESC, updated_at DESC").Find(&arr).Error; err != nil {
			s.respondError(c, 500, "internal_error", "list failed")
			return
		}
	...
	r.POST("/api/support/faq", func(c *gin.Context) {
		...
		var in struct {
			Question, Answer, Category, Tags string
			Visible                          *bool
			Sort                             *int
		}
		if err := c.BindJSON(&in); err != nil || strings.TrimSpace(in.Question) == "" {
			s.respondError(c, 400, "bad_request", "invalid payload")
			return
		}
	...
	r.PUT("/api/support/faq/:id", func(c *gin.Context) {
		...
		var f sup.FAQ
		if err := s.gdb.First(&f, c.Param("id")).Error; err != nil {
			s.respondError(c, 404, "not_found", "not found")
			return
		}
		if in.Question != "" {
			f.Question = in.Question
		}
		if in.Answer != "" {
			f.Answer = in.Answer
		}
		if in.Category != "" || in.Category == "" {
	...
	r.DELETE("/api/support/faq/:id", func(c *gin.Context) {
		...
		if err := s.gdb.Delete(&sup.FAQ{}, c.Param("id")).Error; err != nil {
			s.respondError(c, 500, "internal_error", "delete failed")
			return
		}
		c.Status(204)
	...
	r.DELETE("/api/support/feedback/:id", func(c *gin.Context) {
		...
		if err := s.gdb.Delete(&sup.Feedback{}, c.Param("id")).Error; err != nil {
			s.respondError(c, 500, "internal_error", "delete failed")
			return
		}
		c.Status(204)
