...
func (m *Manager) Verify(tok string) (string, []string, error) {
    ...
    if !hmac.Equal(sig, got) { return "", nil, errors.New("bad signature") }
    cb, err := b64dec(parts[1])
    if err != nil { return "", nil, err }
    var c claims
    if err := json.Unmarshal(cb, &c); err != nil { return "", nil, err }
    if c.Exp > 0 && time.Now().Unix() > c.Exp { return "", nil, errors.New("expired") }
