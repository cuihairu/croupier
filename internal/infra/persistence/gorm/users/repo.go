...
func (r *Repo) Verify(ctx context.Context, username, plain string) (*UserAccount, error) {
    ...
    if err := bcrypt.CompareHashAndPassword([]byte(u.PasswordHash), []byte(plain)); err != nil { return nil, errors.New("invalid credentials") }
    if !u.Active { return nil, errors.New("user disabled") }
    return u, nil
}

// Roles and permissions
func (r *Repo) CreateRole(ctx context.Context, role *RoleRecord) error { return r.db.WithContext(ctx).Create(role).Error }
func (r *Repo) DeleteRole(ctx context.Context, id uint) error { return r.db.WithContext(ctx).Delete(&RoleRecord{}, id).Error }
func (r *Repo) ListRoles(ctx context.Context) ([]*RoleRecord, error) { var arr []*RoleRecord; if err := r.db.WithContext(ctx).Order("id DESC").Find(&arr).Error; err != nil { return nil, err }; return arr, nil }
func (r *Repo) AddUserRole(ctx context.Context, userID, roleID uint) error { return r.db.WithContext(ctx).Create(&UserRoleRecord{UserID: userID, RoleID: roleID}).Error }
...
func (r *Repo) RevokeRolePerm(ctx context.Context, roleID uint, perm string) error { return r.db.WithContext(ctx).Where("role_id=? AND perm=?", roleID, perm).Delete(&RolePermRecord{}).Error }
func (r *Repo) ListRolePerms(ctx context.Context, roleID uint) ([]string, error) { var perms []string; if err := r.db.WithContext(ctx).Model(&RolePermRecord{}).Where("role_id=?", roleID).Pluck("perm", &perms).Error; err != nil { return nil, err }; return perms, nil }
