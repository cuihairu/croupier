...
func New() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			v.AutomaticEnv()
			if v, err = common.ApplySectionAndProfile(v, "agent", profile); err != nil {
				return err
			}
			...
			if err := common.ValidateAgentConfig(v, false); err != nil {
				return err
			}
			...
			if insecureLocal {
				...
				return fmt.Errorf("secure local server not implemented; set --insecure_local")
			}
				...
				go func() {
					...
					for {
						func() {
							...
							if err != nil {
								slog.Warn("assignments poll error", "error", err)
								return
							}
							defer resp.Body.Close()
							if resp.StatusCode/100 != 2 {
								b, _ := io.ReadAll(resp.Body)
								slog.Warn("assignments poll failed", "status", resp.StatusCode, "body", string(b))
								return
							}
							var out struct {
							...
							if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
								slog.Warn("assignments decode", "error", err)
								return
							}
							var fns []string
							for _, arr := range out.Assignments {
								fns = append(fns, arr...)
							}
							...
							if downDir != "" {
								...
								if err := os.MkdirAll(downDir, 0o755); err != nil {
									slog.Warn("downlink dir", "error", err)
								} else {
									...
									if etag, err := downloadAndExtractPack(api+"/api/packs/export", packPath, downDir); err != nil {
										...
										if err := verifyServerPacksList(api, etag, 5*time.Second); err != nil {
											...
											slog.Info("downlink verify packs/list ok")
										}
									}
								}
							}
			...
			go func() {
				...
				r.GET("/metrics", func(c *gin.Context) {
					...
					if promSup != nil {
						adapters["prom"] = promSup.Stats()
					}
					if httpSup != nil {
						adapters["http"] = httpSup.Stats()
					}
				...
				r.POST("/analytics/payments", func(c *gin.Context) {
					...
					case []any:
						for _, it := range v {
							if m, ok := it.(map[string]any); ok {
								_ = anq.PublishPayment(m)
							}
						}
					case map[string]any:
						_ = anq.PublishPayment(v)
					default:
						c.JSON(http.StatusBadRequest, gin.H{"error": "invalid payload"})
						return
					}
...
func (s *adapterSupervisor) SetDesired(want bool) {
	...
	if want {
		...
		if s.running {
			go s.stopGraceful(3 * time.Second)
		}
	}
	s.mu.Unlock()
}
...
func (s *adapterSupervisor) startLoop() {
	s.mu.Lock()
	if s.running || !s.desired || len(s.args) == 0 {
		s.mu.Unlock()
		return
	}
	// spawn process
	...
	if s.logDir != "" {
		...
		if s.stdout == nil {
			s.stdout = newRotatingWriter(filepath.Join(s.logDir, fmt.Sprintf("adapter-%s.out.log", s.name)), s.logMaxBytes, s.logBackups)
		}
		if s.stderr == nil {
			s.stderr = newRotatingWriter(filepath.Join(s.logDir, fmt.Sprintf("adapter-%s.err.log", s.name)), s.logMaxBytes, s.logBackups)
		}
		cmd.Stdout = s.stdout
		cmd.Stderr = s.stderr
	} else {
	...
	if err != nil {
		slog.Error("adapter exited", "name", s.name, "error", err)
	} else {
		slog.Info("adapter exited", "name", s.name)
	}
...
func (s *adapterSupervisor) Stats() AdapterStats {
	...
	defer s.mu.Unlock()
	up := 0
	if s.running && !s.lastStart.IsZero() {
		up = int(time.Since(s.lastStart).Seconds())
	}
	return AdapterStats{
		Name:                s.name,
		Running:             s.running,
...
func calcAdapterNeeds(fns []string) (wantProm, wantHttp bool) {
	for _, id := range fns {
		...
		if strings.HasPrefix(id, "http.") || strings.HasPrefix(id, "grafana.") || strings.HasPrefix(id, "alertmanager.") {
			wantHttp = true
		}
	}
	return
...
func verifyServerPacksList(api string, expectedETag string, timeout time.Duration) error {
	...
	for {
		...
		if err == nil {
			...
			if resp.StatusCode/100 == 2 {
				...
				if out.ETag != "" && out.ETag == expectedETag {
					return nil
				}
			}
		}
	...
	if expectedETag != "" {
		return fmt.Errorf("packs/list not confirmed with expected etag within %s", timeout)
	}
