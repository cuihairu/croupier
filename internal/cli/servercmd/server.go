...
func New() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if v, err = common.ApplySectionAndProfile(v, "server", profile); err != nil {
				return err
			}
			...
			if v.IsSet("db.driver") {
				_ = os.Setenv("DB_DRIVER", v.GetString("db.driver"))
			}
			if v.IsSet("db.dsn") {
				_ = os.Setenv("DATABASE_URL", v.GetString("db.dsn"))
			}
			...
			if err := common.ValidateServerConfig(v, false); err != nil {
			...
			if cert == "" || key == "" || ca == "" {
				...
				if err != nil {
					return fmt.Errorf("generate dev server cert: %w", err)
				}
