...
func (s *PGStore) List(f Filter, p Page) (items []*Approval, total int, err error) {
	...
	order := "DESC"
	if strings.ToLower(p.Sort) == "created_at_asc" {
		order = "ASC"
	}
	...
	if f.FunctionID != "" {
		add("function_id=$%d", f.FunctionID)
	}
	if f.GameID != "" {
		add("game_id=$%d", f.GameID)
	}
	if f.Env != "" {
		add("env=$%d", f.Env)
	}
	...
	for rows.Next() {
		var a Approval
		if err = rows.Scan(&a.ID, &a.CreatedAt, &a.Actor, &a.FunctionID, &a.Payload, &a.IdempotencyKey, &a.Route, &a.TargetServiceID, &a.HashKey, &a.GameID, &a.Env, &a.State, &a.Mode, &a.Reason); err != nil {
			return
		}
		aa := a
