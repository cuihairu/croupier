...
func newAgentLimiter(rps int) *agentLimiter {
	return &agentLimiter{cap: rps, tokens: float64(rps), last: time.Now(), rate: float64(rps)}
}
...
func (s *Server) SetServiceRateLookup(fn func(agentID string) int) {
	s.rlMu.Lock()
	defer s.rlMu.Unlock()
	s.rateLookup = fn
}
