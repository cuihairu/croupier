...
func (s *Server) addOpsRoutes(r *gin.Engine) {
    ...
    r.GET("/api/ops/services", func(c *gin.Context) {
        if _, _, ok := s.require(c, "ops:read", "registry:read"); !ok { return }
        type Agent struct {
            ...
            for _, a := range s.reg.AgentsUnsafe() {
                ...
                if stats != nil {
                    if st := stats[a.AgentID]; st != nil {
                        ...
                        if st.AvgResponseTime > 0 { avgMs = st.AvgResponseTime.Milliseconds() }
                        if tot > 0 { errRate = float64(fail) / float64(tot) }
                        if !st.LastSeen.IsZero() { lastSeen = st.LastSeen.Format(time.RFC3339) }
                        qps1m = st.QPS1m
                    }
                }
    ...
    r.PUT("/api/ops/agents/:id/meta", func(c *gin.Context) {
        ...
        var in struct{ Region string; Zone string }
        if err := c.BindJSON(&in); err != nil { s.respondError(c, 400, "bad_request", "invalid payload"); return }
        if s.reg == nil { s.respondError(c, 503, "unavailable", "registry not available"); return }
        s.reg.Mu().Lock()
        if a := s.reg.AgentsUnsafe()[id]; a != nil {
            if strings.TrimSpace(in.Region) != "" { a.Region = strings.TrimSpace(in.Region) }
            if strings.TrimSpace(in.Zone) != "" { a.Zone = strings.TrimSpace(in.Zone) }
            s.reg.Mu().Unlock()
            c.Status(204)
            return
        }
    ...
    r.POST("/api/agent/meta", func(c *gin.Context) {
        ...
        if tok == "" { s.respondError(c, 503, "unavailable", "agent meta disabled"); return }
        if c.Request.Header.Get("X-Agent-Token") != tok { s.respondError(c, 401, "unauthorized", "unauthorized"); return }
        var in struct{ AgentID, Region, Zone string; Labels map[string]string }
        if err := c.BindJSON(&in); err != nil || strings.TrimSpace(in.AgentID) == "" { s.respondError(c, 400, "bad_request", "invalid payload"); return }
        if s.reg == nil { s.respondError(c, 503, "unavailable", "registry not available"); return }
        s.reg.Mu().Lock()
        if a := s.reg.AgentsUnsafe()[in.AgentID]; a != nil {
            if v := strings.TrimSpace(in.Region); v != "" { a.Region = v }
            if v := strings.TrimSpace(in.Zone); v != "" { a.Zone = v }
            if in.Labels != nil {
                if a.Labels == nil { a.Labels = map[string]string{} }
                for k, v := range in.Labels { a.Labels[k] = v }
            }
            s.reg.Mu().Unlock()
            c.Status(204)
            return
    ...
    r.GET("/api/ops/rate-limits", func(c *gin.Context) {
        ...
        type Rule struct { Scope string `json:"scope"`; Key string `json:"key"`; LimitQPS int `json:"limit_qps"`; Match map[string]string `json:"match,omitempty"`; Percent int `json:"percent,omitempty"` }
        out := []Rule{}
        s.mu.Lock()
        if len(s.fnRulesAdv)>0 || len(s.svcRulesAdv)>0 {
            for _, rr := range s.fnRulesAdv { out = append(out, Rule{Scope: "function", Key: rr.Key, LimitQPS: rr.LimitQPS, Match: rr.Match, Percent: rr.Percent}) }
            for _, rr := range s.svcRulesAdv { out = append(out, Rule{Scope: "service", Key: rr.Key, LimitQPS: rr.LimitQPS, Match: rr.Match, Percent: rr.Percent}) }
        } else {
            for k, v := range s.rateLimitRules { out = append(out, Rule{Scope: "function", Key: k, LimitQPS: v, Percent: 100}) }
            for k, v := range s.serviceRateRules { out = append(out, Rule{Scope: "service", Key: k, LimitQPS: v, Percent: 100}) }
        }
        s.mu.Unlock()
    ...
    r.PUT("/api/ops/rate-limits", func(c *gin.Context) {
        ...
        if err := c.BindJSON(&in); err != nil || len(in.Rules) == 0 { s.respondError(c, 400, "bad_request", "invalid payload"); return }
        // snapshot old set for diff
        s.mu.Lock()
        oldKeys := []string{}
        if len(s.fnRulesAdv)>0 || len(s.svcRulesAdv)>0 {
            for _, rr := range s.fnRulesAdv { oldKeys = append(oldKeys, "function:"+rr.Key) }
            for _, rr := range s.svcRulesAdv { oldKeys = append(oldKeys, "service:"+rr.Key) }
        } else {
            for k := range s.rateLimitRules { oldKeys = append(oldKeys, "function:"+k) }
            for k := range s.serviceRateRules { oldKeys = append(oldKeys, "service:"+k) }
        }
        ...
        for _, r0 := range in.Rules {
            ...
            if k == "" || r0.LimitQPS <= 0 { continue }
            pct := r0.Percent; if pct <= 0 { pct = 100 }
            rr := rateRuleAdv{Scope: scope, Key: k, LimitQPS: r0.LimitQPS, Match: r0.Match, Percent: pct}
            if scope == "function" { s.fnRulesAdv = append(s.fnRulesAdv, rr) } else if scope == "service" { s.svcRulesAdv = append(s.svcRulesAdv, rr) }
        }
        ...
        newKeys := []string{}
        for _, rr := range s.fnRulesAdv { newKeys = append(newKeys, "function:"+rr.Key) }
        for _, rr := range s.svcRulesAdv { newKeys = append(newKeys, "service:"+rr.Key) }
        s.mu.Unlock()
        if err := s.saveRateLimitsToFile(); err != nil { s.respondError(c, 500, "internal_error", "persist failed"); return }
        ...
        if setter, ok := s.invoker.(interface{ SetServiceRateLookup(func(string) int) }); ok {
            setter.SetServiceRateLookup(func(agentID string) int {
                ...
                if s.reg != nil {
                    s.reg.Mu().RLock()
                    if a := s.reg.AgentsUnsafe()[agentID]; a != nil { gid, env, regn, zone, labels = a.GameID, a.Env, a.Region, a.Zone, a.Labels }
                    s.reg.Mu().RUnlock()
                }
                // choose best matching service rule
                best := 0; pct := 100; score := -1
                for _, rr := range s.svcRulesAdv {
                    if rr.Key != "" && rr.Key != agentID { continue }
                    m := rr.Match
                    sc := 0
                    if m != nil {
                        if v:=strings.TrimSpace(m["game_id"]); v != "" { if v!=gid { continue } ; sc++ }
                        if v:=strings.TrimSpace(m["env"]); v != "" { if v!=env { continue } ; sc++ }
                        if v:=strings.TrimSpace(m["region"]); v != "" { if v!=regn { continue } ; sc++ }
                        if v:=strings.TrimSpace(m["zone"]); v != "" { if v!=zone { continue } ; sc++ }
                        // other keys treated as label match
                        for k, v := range m {
                            if k=="game_id"||k=="env"||k=="region"||k=="zone" { continue }
                            if labels == nil { labels = map[string]string{} }
                            if labels[k] != v { sc = -1; break }
                        }
                        if sc < 0 { continue }
                    }
                    if sc > score { best = rr.LimitQPS; pct = rr.Percent; score = sc }
                }
                if best <= 0 { // fallback map
                    if v := s.serviceRateRules[agentID]; v > 0 { best = v; pct = 100 }
                }
                if best <= 0 { return 0 }
                if pct < 100 { best = best * pct / 100 }
                if best <= 0 { best = 1 }
                return best
            })
        }
        ...
        if s.audit != nil {
            // compute diff counts
            oldSet := map[string]struct{}{}; for _, k := range oldKeys { oldSet[k]=struct{}{} }
            newSet := map[string]struct{}{}; for _, k := range newKeys { newSet[k]=struct{}{} }
            added, removed := 0,0
            for k := range newSet { if _, ok := oldSet[k]; !ok { added++ } }
            for k := range oldSet { if _, ok := newSet[k]; !ok { removed++ } }
    ...
    r.DELETE("/api/ops/rate-limits", func(c *gin.Context) {
        ...
        if key == "" || (scope != "function" && scope != "service") { s.respondError(c, 400, "bad_request", "invalid scope/key"); return }
        s.mu.Lock()
        if scope == "function" { delete(s.rateLimitRules, key) } else { delete(s.serviceRateRules, key) }
        s.mu.Unlock()
        if err := s.saveRateLimitsToFile(); err != nil { s.respondError(c, 500, "internal_error", "persist failed"); return }
        if setter, ok := s.invoker.(interface{ SetServiceRateLookup(func(string) int) }); ok {
            setter.SetServiceRateLookup(func(agentID string) int { s.mu.RLock(); defer s.mu.RUnlock(); return s.serviceRateRules[agentID] })
        }
        if s.audit != nil { _ = s.audit.Log("ops.rate_limit_update", user, scope+":"+key, map[string]string{"ip": c.ClientIP(), "action": "delete"}) }
        c.Status(204)
    })
    ...
    r.GET("/api/ops/rate-limits/preview", func(c *gin.Context) {
        ...
        if scope != "service" { s.JSON(c, 200, gin.H{"matched": 0, "agents": []any{}}); return }
        if s.reg == nil { s.JSON(c, 200, gin.H{"matched": 0, "agents": []any{}}); return }
        var stats map[string]*loadbalancer.AgentStats
        if s.statsProv != nil { stats = s.statsProv.GetStats() }
        s.reg.Mu().RLock()
        out := []map[string]any{}
        for _, a := range s.reg.AgentsUnsafe() {
            if key != "" && a.AgentID != key { continue }
            if mg != "" && a.GameID != mg { continue }
            if me != "" && a.Env != me { continue }
            if mr != "" && a.Region != mr { continue }
            if mz != "" && a.Zone != mz { continue }
            eff := limitQPS
            if pct > 0 && pct < 100 { eff = eff * pct / 100 }
            if eff <= 0 { eff = limitQPS }
            cur := 0.0
            if stats != nil { if st := stats[a.AgentID]; st != nil { cur = st.QPS1m } }
    ...
    r.GET("/api/ops/functions", func(c *gin.Context) {
        if _, _, ok := s.require(c, "ops:read", "registry:read"); !ok { return }
        type Fn struct{ ID string `json:"id"`; Category string `json:"category,omitempty"` }
    ...
    r.GET("/api/ops/jobs", func(c *gin.Context) {
        ...
        for k, v := range s.jobs { cp := *v; jm[k] = &cp }
        s.jobsMu.Unlock()
        // newest first
        // filter and collect
        ...
        for i := len(order)-1; i >= 0; i-- {
            ...
            ji := jm[id]
            if ji == nil { continue }
            if status != "" && ji.State != status { continue }
            if fid != "" && ji.FunctionID != fid { continue }
            if actor != "" && ji.Actor != actor { continue }
            if gid != "" && ji.GameID != gid { continue }
            if env != "" && ji.Env != env { continue }
            list = append(list, ji)
        }
        total := len(list)
        start := (page-1)*size
        ...
        for _, ji := range window {
            out = append(out, map[string]any{
                ...
                "ended_at": func() string { if ji.EndedAt.IsZero() { return "" } else { return ji.EndedAt.Format(time.RFC3339) } }(),
    ...
    r.GET("/api/ops/alerts", func(c *gin.Context) {
        ...
        u := am
        if !strings.HasSuffix(u, "/") { u += "/" }
        u += "api/v2/alerts"
        req, _ := http.NewRequestWithContext(c.Request.Context(), http.MethodGet, u, nil)
        if v := strings.TrimSpace(os.Getenv("ALERTMANAGER_BEARER")); v != "" { req.Header.Set("Authorization", "Bearer "+v) }
        to := 1500 * time.Millisecond
        if tv := strings.TrimSpace(os.Getenv("ALERTMANAGER_TIMEOUT_MS")); tv != "" { if n, err := strconv.Atoi(tv); err==nil && n>0 { to = time.Duration(n)*time.Millisecond } }
        cli := &http.Client{ Timeout: to }
        resp, err := cli.Do(req)
        if err != nil || resp == nil || resp.Body == nil { s.respondError(c, 502, "bad_gateway", "alertmanager unavailable"); return }
        defer resp.Body.Close()
        var arr []map[string]any
        if err := json.NewDecoder(resp.Body).Decode(&arr); err != nil { s.respondError(c, 502, "bad_gateway", "invalid alertmanager response"); return }
        ...
        for _, a := range arr {
            ...
            sev := ""; if labels!=nil { if v,ok:=labels["severity"].(string); ok { sev=v } }
            inst := ""; if labels!=nil { if v,ok:=labels["instance"].(string); ok { inst=v } }
            service := ""; if labels!=nil { if v,ok:=labels["service"].(string); ok { service=v } else if v2,ok2:=labels["job"].(string); ok2 { service=v2 } }
            summary := ""; if ann!=nil { if v,ok:=ann["summary"].(string); ok { summary=v } }
            startsAt := ""; if v,ok := a["startsAt"].(string); ok { startsAt=v }
            endsAt := ""; if v,ok := a["endsAt"].(string); ok { endsAt=v }
            silenced := false; if status!=nil { if s2,ok:=status["state"].(string); ok && s2=="suppressed" { silenced = true } }
            dur := ""
            if t, err := time.Parse(time.RFC3339Nano, startsAt); err==nil { dur = now.Sub(t).Truncate(time.Second).String() }
            out = append(out, map[string]any{
    ...
    r.POST("/api/ops/alerts/silence", func(c *gin.Context) {
        if _, _, ok := s.require(c, "ops:manage"); !ok { return }
        am := strings.TrimSpace(os.Getenv("ALERTMANAGER_URL"))
        if am == "" { s.respondError(c, 503, "unavailable", "alertmanager not configured"); return }
        var in struct{ Matchers map[string]string; Duration string; Creator string; Comment string }
        if err := c.BindJSON(&in); err != nil { s.respondError(c, 400, "bad_request", "invalid payload"); return }
        if in.Duration == "" { in.Duration = "1h" }
        d, err := time.ParseDuration(in.Duration); if err != nil { s.respondError(c, 400, "bad_request", "invalid duration"); return }
        now := time.Now().UTC()
        ms := []map[string]any{}
        for k,v := range in.Matchers { ms = append(ms, map[string]any{"name": k, "value": v, "isRegex": false}) }
        ...
        u := am; if !strings.HasSuffix(u, "/") { u += "/" }; u += "api/v2/silences"
        ...
        cli := &http.Client{ Timeout: to }
        resp, err := cli.Do(req)
        if err != nil { s.respondError(c, 502, "bad_gateway", "alertmanager unavailable"); return }
        defer resp.Body.Close()
        if resp.StatusCode/100 != 2 { s.respondError(c, 502, "bad_gateway", "silence create failed"); return }
        c.Status(204)
    })
    r.GET("/api/ops/alerts/silences", func(c *gin.Context) {
        if _, _, ok := s.require(c, "ops:read"); !ok { return }
        am := strings.TrimSpace(os.Getenv("ALERTMANAGER_URL"))
        if am == "" { s.JSON(c, 200, gin.H{"silences": []any{}}); return }
    ...
    r.DELETE("/api/ops/alerts/silences/:id", func(c *gin.Context) {
        ...
        if err != nil { s.respondError(c, 502, "bad_gateway", "alertmanager unavailable"); return }
        defer resp.Body.Close()
        if resp.StatusCode/100 != 2 { s.respondError(c, 502, "bad_gateway", "delete failed"); return }
        c.Status(204)
    })
    ...
    r.GET("/api/ops/metrics", func(c *gin.Context) {
        ...
        if qQPS == "" { qQPS = `sum(rate(http_requests_total{instance="{instance}"}[1m]))` }
        if qERR == "" { qERR = `sum(rate(http_requests_total{instance="{instance}",status=~"5.."}[5m])) / sum(rate(http_requests_total{instance="{instance}"}[5m]))` }
        if qP95 == "" { qP95 = `histogram_quantile(0.95, sum(rate(http_request_duration_seconds_bucket{instance="{instance}"}[5m])) by (le))` }
        // timeframe
        dur, err := time.ParseDuration(rng)
        if err != nil { s.respondError(c, 400, "bad_request", "invalid range"); return }
        end := time.Now()
        start := end.Add(-dur)
        // helper to call prom
        ...
        doRange := func(query string) ([][2]any, error) {
            ...
            req, _ := http.NewRequestWithContext(c.Request.Context(), http.MethodGet, u, nil)
            if v := strings.TrimSpace(os.Getenv("PROM_BEARER")); v != "" { req.Header.Set("Authorization", "Bearer "+v) }
            to := 2000 * time.Millisecond
            if tv := strings.TrimSpace(os.Getenv("PROM_TIMEOUT_MS")); tv != "" { if n, err := strconv.Atoi(tv); err==nil && n>0 { to = time.Duration(n)*time.Millisecond } }
            cli := &http.Client{ Timeout: to }
            resp, err := cli.Do(req)
            if err != nil || resp == nil || resp.Body == nil { return nil, err }
            defer resp.Body.Close()
            var pr struct{ Status string; Data struct{ ResultType string `json:"resultType"`; Result []struct{ Values [][2]any `json:"values"` } `json:"result"` } }
            if err := json.NewDecoder(resp.Body).Decode(&pr); err != nil { return nil, err }
            if len(pr.Data.Result) == 0 { return [][2]any{}, nil }
