...
func (fakeInvoker) CancelJob(ctx context.Context, req *functionv1.CancelJobRequest) (*functionv1.StartJobResponse, error) {
	return &functionv1.StartJobResponse{JobId: ""}, nil
}
...
func TestPacks_List_And_UISchema_Export(t *testing.T) {
	...
	if rr.Code/100 != 2 {
		t.Fatalf("packs list code=%d body=%s", rr.Code, rr.Body.String())
	}
	...
	if out["manifest"] == nil {
		t.Fatalf("missing manifest in list")
	}
	if etg, _ := out["etag"].(string); etg == "" {
		t.Fatalf("missing etag in packs/list")
	}
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("ui_schema code=%d body=%s", rr.Code, rr.Body.String())
	}
	...
	if et := rr.Header().Get("ETag"); et == "" {
		t.Fatalf("missing ETag header on export")
	}
	// read tar.gz entries
	...
	if !foundMani {
		t.Fatalf("export missing manifest.json")
	}
}
...
func TestAssignments_Get_Post_And_PacksReload(t *testing.T) {
	...
	if rr.Code != http.StatusUnauthorized {
		t.Fatalf("assignments get expect 401, got %d", rr.Code)
	}
	rr = httptest.NewRecorder()
	req.Header.Set("Authorization", "Bearer "+tok)
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("assignments get code=%d body=%s", rr.Code, rr.Body.String())
	}
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("assignments post code=%d body=%s", rr.Code, rr.Body.String())
	}
	var postOut struct {
		Ok      bool     `json:"ok"`
		Unknown []string `json:"unknown"`
	}
	if err := json.Unmarshal(rr.Body.Bytes(), &postOut); err != nil {
		t.Fatalf("json: %v", err)
	}
	if len(postOut.Unknown) != 1 || postOut.Unknown[0] != "bad.fn" {
		t.Fatalf("unexpected unknown: %+v", postOut.Unknown)
	}
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("assignments get2 code=%d body=%s", rr.Code, rr.Body.String())
	}
	var getOut struct {
		Assignments map[string][]string `json:"assignments"`
	}
	if err := json.Unmarshal(rr.Body.Bytes(), &getOut); err != nil {
		t.Fatalf("json: %v", err)
	}
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("descriptors code=%d body=%s", rr.Code, rr.Body.String())
	}
	var descs []map[string]any
	if err := json.Unmarshal(rr.Body.Bytes(), &descs); err != nil {
		t.Fatalf("json: %v", err)
	}
	...
	if !(have["ex.fn"] && have["ex2.fn"]) {
		t.Fatalf("expected ex.fn and ex2.fn after reload, got: %+v", have)
	}
}
...
func TestPacksReload_RBAC(t *testing.T) {
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("expected 2xx, got %d", rr.Code)
	}
}
...
func TestRegistry_Coverage_HealthyTotal_Uncovered(t *testing.T) {
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("registry code=%d body=%s", rr.Code, rr.Body.String())
	}
	var out struct {
		Coverage []struct {
	...
	if cov == nil {
		t.Fatalf("missing coverage for g1|dev")
	}
	if cov["ex.fn"].Healthy != 1 || cov["ex.fn"].Total != 2 {
		t.Fatalf("unexpected ex.fn cov: %+v", cov["ex.fn"])
	}
	if cov["ex2.fn"].Healthy != 0 || cov["ex2.fn"].Total != 0 {
		t.Fatalf("unexpected ex2.fn cov: %+v", cov["ex2.fn"])
	}
	...
	if !found {
		t.Fatalf("ex2.fn should be in uncovered: %+v", unc)
	}
}
...
func TestRegistry_RBAC(t *testing.T) {
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("expect 403, got %d", rr.Code)
	}
	// allowed
	tokOK, _ := mgr.Sign("ok", nil, 0)
	rr = httptest.NewRecorder()
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("expect 2xx, got %d", rr.Code)
	}
}
...
func TestAudit_RBAC(t *testing.T) {
	...
	srv.ginEngine().ServeHTTP(rr, req)
	if rr.Code/100 != 2 {
		t.Fatalf("expect 2xx, got %d body=%s", rr.Code, rr.Body.String())
	}
}
