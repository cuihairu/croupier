...
func (s *Server) addCertificateRoutes(r *gin.Engine) {
    ...
    certGroup.Use(func(c *gin.Context) {
        if _, _, ok := s.require(c, "certificates:manage"); !ok { return }
        c.Next()
    })
	...
	certGroup.POST("", func(c *gin.Context) {
        ...
        if err := c.ShouldBindJSON(&req); err != nil { s.respondError(c, 400, "bad_request", err.Error()); return }
        ...
        if err := s.certStore.AddDomain(req.Domain, req.Port, req.AlertDays); err != nil { s.respondError(c, 500, "internal_error", err.Error()); return }
	...
	certGroup.POST("/:id/check", func(c *gin.Context) {
        ...
        if err != nil { s.respondError(c, 400, "bad_request", "invalid certificate id"); return }

        if err := s.certStore.CheckCertificate(uint(id)); err != nil { s.respondError(c, 500, "internal_error", err.Error()); return }
	...
	certGroup.POST("/check-all", func(c *gin.Context) {
        if err := s.certStore.CheckAllCertificates(); err != nil { s.respondError(c, 500, "internal_error", err.Error()); return }
	...
	certGroup.POST("/:id/alerts", func(c *gin.Context) {
        ...
        if err := s.certStore.AddAlert(uint(id), req.AlertType, req.Target); err != nil { s.respondError(c, 500, "internal_error", err.Error()); return }
