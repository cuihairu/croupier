...
func EnsureDevCA(dir string) (caCrt, caKey string, err error) {
    ...
    if _, e1 := os.Stat(caCrt); e1 == nil {
        if _, e2 := os.Stat(caKey); e2 == nil { return caCrt, caKey, nil }
    }
    ...
    if err := writeFile(caCrt, crtPEM, 0o644); err != nil { return "", "", err }
    if err := writeFile(caKey, keyPEM, 0o600); err != nil { return "", "", err }
...
func EnsureServerCert(dir, caCrtPath, caKeyPath string, hosts []string) (crtPath, keyPath string, err error) {
    ...
    if _, e1 := os.Stat(crtPath); e1 == nil {
        if _, e2 := os.Stat(keyPath); e2 == nil { return crtPath, keyPath, nil }
    }
    ...
    if caCrtBlock == nil || caKeyBlock == nil { return "", "", fmt.Errorf("invalid CA pem files") }
    ...
    crtPEM := pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: der})
    keyPEM := pem.EncodeToMemory(&pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(key)})
    if err := writeFile(crtPath, crtPEM, 0o644); err != nil { return "", "", err }
