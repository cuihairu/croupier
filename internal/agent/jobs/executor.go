...
func NewExecutor() *Executor {
	return &Executor{ch: map[string]chan *functionv1.JobEvent{}, idem: map[string]idemEntry{}, cancel: map[string]context.CancelFunc{}, results: map[string]JobStatus{}}
}
...
func (e *Executor) Start(ctx context.Context, req *functionv1.InvokeRequest, localAddr string) (string, bool) {
	...
	e.ch[jobID] = ch
	if req.GetIdempotencyKey() != "" {
		e.idem[req.GetIdempotencyKey()] = idemEntry{jobID: jobID, expireAt: now.Add(idemWindow)}
	}
	e.mu.Unlock()
	...
	go func() {
		...
		emit := func(ev *functionv1.JobEvent) {
			select {
			case ch <- ev:
			default:
			}
		...
		if err != nil {
			emit(&functionv1.JobEvent{Type: "error", Message: err.Error()})
			e.mu.Lock()
			e.results[jobID] = JobStatus{State: "error", Error: err.Error(), CompletedAt: time.Now()}
			e.mu.Unlock()
			return
		}
		...
		e.mu.Lock()
		e.results[jobID] = JobStatus{State: "done", Payload: resp.GetPayload(), CompletedAt: time.Now()}
		e.mu.Unlock()
	}()
...
func (e *Executor) Status(jobID string) (JobStatus, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	if st, ok := e.results[jobID]; ok {
		return st, true
	}
	if _, ok := e.ch[jobID]; ok {
		return JobStatus{State: "running"}, true
	}
