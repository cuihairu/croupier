...
func (c *Client) Start(ctx context.Context) error {
	...
	// send hello
	if err := stream.Send(&tunnelv1.TunnelMessage{Type: "hello", Hello: &tunnelv1.Hello{AgentId: c.agentID, GameId: c.gameID, Env: c.env}}); err != nil {
		return err
	}
	// heartbeat sender
	...
	for {
		...
		if err != nil {
			close(done)
			slog.Warn("tunnel recv end", "error", err.Error())
			return err
		}
		if msg == nil {
			continue
		...
		case "list_local_req":
			...
			if err != nil || resp == nil {
				...
				for _, lf := range resp.Functions {
					if lf.Id == lr.FunctionId {
						for _, inst := range lf.Instances {
							ids = append(ids, inst.ServiceId)
						}
					}
				}
