...
func (r *Repo) ensureEnvDef(ctx context.Context, env, desc, color string) error {
	...
	changed := false
	if d := strings.TrimSpace(desc); d != "" && strings.TrimSpace(ge.Description) != d {
		ge.Description = d
		changed = true
	}
	if c := strings.TrimSpace(color); c != "" && strings.TrimSpace(ge.Color) != c {
		ge.Color = c
		changed = true
	}
	if changed {
...
func (r *Repo) AddEnv(ctx context.Context, gameID uint, env string) error {
	return r.AddEnvWithMeta(ctx, gameID, env, "", "")
}
func (r *Repo) AddEnvWithDesc(ctx context.Context, gameID uint, env, desc string) error {
	return r.AddEnvWithMeta(ctx, gameID, env, desc, "")
}
...
func (r *Repo) UpdateEnv(ctx context.Context, gameID uint, oldEnv, newEnv, desc, color string) error {
	...
	if newEnv != "" && !strings.EqualFold(oldEnv, newEnv) {
		for i, e := range lst {
			if strings.EqualFold(strings.TrimSpace(e), oldEnv) {
				lst[i] = newEnv
				changed = true
			}
		}
