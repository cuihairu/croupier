...
func (p *PortRepo) Create(ctx context.Context, g *dom.Game) error {
	...
	if err := p.r.Create(ctx, m); err != nil {
		return err
	}
	g.ID = m.ID
	return nil
...
func (p *PortRepo) ListEnvs(ctx context.Context, gameID uint) ([]string, error) {
	return p.r.ListEnvs(ctx, gameID)
}
...
func (p *PortRepo) AddEnv(ctx context.Context, gameID uint, env string) error {
	return p.r.AddEnv(ctx, gameID, env)
}
...
func (p *PortRepo) RemoveEnv(ctx context.Context, gameID uint, env string) error {
	return p.r.RemoveEnv(ctx, gameID, env)
}
