...
func NewServer() *Server {
	return &Server{agents: map[string]*edgeConn{}, pending: map[string]*pending{}, pendingList: map[string]chan *tunnelv1.ListLocalResponse{}, pendingJobRes: map[string]chan *tunnelv1.GetJobResultResponse{}, subs: map[string][]chan *functionv1.JobEvent{}, jobs: map[string]string{}, results: map[string]JobResult{}}
}

func (s *Server) Open(stream tunnelv1.TunnelService_OpenServer) error {
	// expect Hello first
	hello, err := stream.Recv()
	if err != nil || hello == nil || hello.Type != "hello" || hello.Hello == nil {
		return errors.New("bad hello")
	}
	h := hello.Hello
	conn := &edgeConn{agentID: h.AgentId, gameID: h.GameId, env: h.Env, srv: stream, last: time.Now()}
	s.mu.Lock()
	s.agents[h.AgentId] = conn
	s.mu.Unlock()
	...
	for {
		...
		case "job_event":
			if msg.JobEvt != nil {
				...
				if ev.Type == "done" || ev.Type == "error" {
					...
					s.evMu.Lock()
					for _, ch := range s.subs[ev.JobId] {
						close(ch)
					}
					...
					s.jobsMu.Lock()
					delete(s.jobs, ev.JobId)
					s.jobsMu.Unlock()
					// save final result snapshot
					st := JobResult{State: ev.Type}
					if ev.Type == "done" {
						st.Payload = ev.Payload
					} else {
						st.Error = ev.Message
					}
					s.resMu.Lock()
					s.results[ev.JobId] = st
					s.resMu.Unlock()
				}
			}
		case "heartbeat":
			s.mu.Lock()
			if c := s.agents[h.AgentId]; c != nil {
				c.last = time.Now()
			}
			s.mu.Unlock()
		case "list_local_res":
			if msg.ListRes != nil {
				s.lpMu.Lock()
				if ch := s.pendingList[msg.ListRes.RequestId]; ch != nil {
					select {
					case ch <- msg.ListRes:
					default:
					}
		...
		case "get_job_result_res":
			if msg.JobResRes != nil {
				s.jrMu.Lock()
				if ch := s.pendingJobRes[msg.JobResRes.RequestId]; ch != nil {
					select {
					case ch <- msg.JobResRes:
					default:
					}
				}
				s.jrMu.Unlock()
...
func (s *Server) InvokeViaTunnel(agentID, requestID, functionID string, idem string, payload []byte, meta map[string]string) (*tunnelv1.ResultFrame, error) {
	...
	s.pendMu.Lock()
	s.pending[requestID] = &pending{ch: ch, created: time.Now()}
	s.pendMu.Unlock()
	...
	if err := conn.srv.Send(msg); err != nil {
		return nil, err
	}
...
func (s *Server) GetJobResultViaTunnel(agentID, requestID, jobID string) (JobResult, error) {
	...
	s.mu.RUnlock()
	if conn == nil {
		return JobResult{}, errors.New("agent not connected")
	}
	ch := make(chan *tunnelv1.GetJobResultResponse, 1)
	s.jrMu.Lock()
	s.pendingJobRes[requestID] = ch
	s.jrMu.Unlock()
