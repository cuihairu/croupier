...
func TestCreateGameAppliesDefaults(t *testing.T) {
	...
	if g.ID == 0 {
		t.Fatalf("expected game ID assigned")
	}
...
func TestEnvCRUDViaService(t *testing.T) {
	...
	// Add env with meta
	if err := svc.AddEnv(context.Background(), g.ID, dom.GameEnvDef{Env: "prod", Description: "生产", Color: "#fff"}); err != nil {
		t.Fatal(err)
	}
	// Update env meta and rename
	if err := svc.UpdateEnv(context.Background(), g.ID, "prod", dom.GameEnvDef{Env: "production", Description: "生产环境", Color: "#000"}); err != nil {
		t.Fatal(err)
	}
	// Remove
	if err := svc.RemoveEnv(context.Background(), g.ID, "production"); err != nil {
		t.Fatal(err)
	}
	// Should be empty
	...
	if len(lst) != 0 {
		t.Fatalf("expected 0 envs, got %d", len(lst))
	}
}
