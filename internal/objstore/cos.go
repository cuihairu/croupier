...
func OpenCOS(_ context.Context, c Config) (Store, error) {
    ...
    if c.Endpoint != "" {
        ...
        if !strings.Contains(u.Host, c.Bucket) {
            if !strings.HasSuffix(u.Path, "/"+c.Bucket) { u.Path = "/" + c.Bucket }
        }
        bucketURL = u
    } else {
        if c.Region == "" { return nil, fmt.Errorf("region required for cos when endpoint empty") }
...
func (s *cosStore) Put(ctx context.Context, key string, r ReadSeeker, _ int64, contentType string) error {
    ...
    if contentType != "" { opt.ObjectPutHeaderOptions = &cos.ObjectPutHeaderOptions{ContentType: contentType} }
...
func (s *cosStore) SignedURL(ctx context.Context, key string, method string, expiry time.Duration) (string, error) {
    ...
    case http.MethodPut: m = http.MethodPut
    case http.MethodDelete: m = http.MethodDelete
