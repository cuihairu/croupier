...
type ossStore struct { bk *oss.Bucket; ttl time.Duration }
...
func (s *ossStore) Put(_ context.Context, key string, r ReadSeeker, _ int64, contentType string) error {
    key = sanitizeKey(key)
    opts := []oss.Option{}
    if contentType != "" { opts = append(opts, oss.ContentType(contentType)) }
...
func (s *ossStore) SignedURL(_ context.Context, key string, method string, expiry time.Duration) (string, error) {
    ...
    switch method {
    case "PUT": httpMethod = oss.HTTPPut
    case "DELETE": httpMethod = oss.HTTPDelete
    case "GET", "": httpMethod = oss.HTTPGet
    default:
