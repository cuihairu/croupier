...
func componentEntryToDTO(cfg config.Config, entry componentEntry) Component {
...
func buildComponentList(cfg config.Config, cm *pack.ComponentManager) ([]Component, int, int, int) {
	total := len(cm.ListInstalled()) + len(cm.ListDisabled())
	items := make([]Component, 0, total)
	installedCount := 0
	disabledCount := 0
	functionsCount := 0
