...
func TestHandler_Create_BindValidation(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			body:     `{"username":"newadmin","password":"Password@123","nickname":"New Admin","email":"new@example.com"}`,
			wantCode: 0, // Will fail at service layer
		},
		{
			name:     "missing username",
			body:     `{"password":"Password@123","nickname":"New Admin"}`,
			wantCode: 400,
		},
		{
		...
		{
			name:     "with roles",
			body:     `{"username":"newadmin","password":"Password@123","roles":["admin","editor"]}`,
			wantCode: 0,
		},
	}
