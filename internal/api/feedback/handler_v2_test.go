...
func TestServiceV2_List_WithFilters(t *testing.T) {
	...
	for _, cat := range []string{"bug", "praise", "bug"} {
		err := fbModel.Create(context.TODO(), &model.Feedback{
			Contact: "x@x.com", Content: "c", Category: cat, Status: dbenum.FeedbackStatusOpen, Priority: "normal",
		})
		require.NoError(t, err)
	}

	resp, err := s.List(context.TODO(), &FeedbackListRequest{Category: "bug", Page: 1, PageSize: 10})
	require.NoError(t, err)
	assert.GreaterOrEqual(t, len(resp.Items), 1)

	resp2, err := s.List(context.TODO(), &FeedbackListRequest{Status: "open", Page: 1, PageSize: 10})
	require.NoError(t, err)
	assert.GreaterOrEqual(t, len(resp2.Items), 1)

	resp3, err := s.List(context.TODO(), &FeedbackListRequest{GameId: "demo", Page: 1, PageSize: 10})
	require.NoError(t, err)
...
func TestServiceV2_Create_NilModel(t *testing.T) {
	s := NewService(&svc.ServiceContext{})
	_, err := s.Create(context.TODO(), nil)
	assert.Error(t, err)
}
...
func TestServiceV2_Create_EmptyContent(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	_, err := s.Create(context.TODO(), &FeedbackCreateRequest{Contact: "x", Category: "c"})
	assert.Error(t, err)
...
func TestServiceV2_Create_EmptyCategory(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	_, err := s.Create(context.TODO(), &FeedbackCreateRequest{Contact: "x", Content: "c"})
	assert.Error(t, err)
...
func TestServiceV2_Create_Success_AllFields(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	resp, err := s.Create(context.TODO(), &FeedbackCreateRequest{
...
func TestServiceV2_Update_NilModel(t *testing.T) {
	s := NewService(&svc.ServiceContext{})
	_, err := s.Update(context.TODO(), nil)
	assert.Error(t, err)
}
...
func TestServiceV2_Update_NoFieldsToUpdate(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	_, err := s.Update(context.TODO(), &FeedbackUpdateRequest{ID: "1"})
	assert.Error(t, err)
...
func TestServiceV2_Delete_Success(t *testing.T) {
	...
	require.NoError(t, err)
	err = s.Delete(context.TODO(), &FeedbackDeleteRequest{ID: fmt.Sprintf("%d", createResp.Id)})
	require.NoError(t, err)
}

func TestServiceV2_Stats_NilModel(t *testing.T) {
	s := NewService(&svc.ServiceContext{})
	_, err := s.Stats(context.TODO(), nil)
	assert.Error(t, err)
}
...
func TestServiceV2_Stats_NilRequest_DefaultDays(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	resp, err := s.Stats(context.TODO(), nil)
	require.NoError(t, err)
	require.NotNil(t, resp)
...
func TestServiceV2_Stats_WithDays(t *testing.T) {
	...
	resp, err := s.Stats(context.TODO(), &FeedbackStatsRequest{Days: 30})
	require.NoError(t, err)
	require.NotNil(t, resp)
...
func TestServiceV2_Stats_ZeroDays(t *testing.T) {
	db := newFeedbackTestDBV2(t)
	s := NewService(&svc.ServiceContext{FeedbackModel: model.NewFeedbackModel(db)})
	resp, err := s.Stats(context.TODO(), &FeedbackStatsRequest{Days: 0})
	require.NoError(t, err)
	assert.NotNil(t, resp)
}
