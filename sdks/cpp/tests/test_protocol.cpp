...
TEST_F(ProtocolTest, IsRequest) {
    ...
    EXPECT_TRUE(IsRequest(MSG_START_JOB_REQUEST));
    EXPECT_TRUE(IsRequest(MSG_CANCEL_JOB_REQUEST));
...
TEST_F(ProtocolTest, IsRequestEventMessages) {
    // MSG_JOB_EVENT and MSG_METRIC_EVENT are even but not responses
    EXPECT_FALSE(IsRequest(MSG_JOB_EVENT));
...
TEST_F(ProtocolTest, IsResponseEventMessages) {
    // MSG_JOB_EVENT and MSG_METRIC_EVENT are even but not responses
    EXPECT_FALSE(IsResponse(MSG_JOB_EVENT));
...
TEST_F(ProtocolTest, GetResponseMsgID) {
    ...
    EXPECT_EQ(GetResponseMsgID(MSG_START_JOB_REQUEST), MSG_START_JOB_RESPONSE);
}
...
TEST_F(ProtocolTest, MsgIDStringKnown) {
    ...
    EXPECT_EQ(MsgIDString(MSG_START_JOB_REQUEST), "StartTaskRequest");
    EXPECT_EQ(MsgIDString(MSG_JOB_EVENT), "TaskEvent");
...
// Test MSG_JOB_EVENT and MSG_METRIC_EVENT special handling
...
TEST_F(ProtocolTest, ClientServiceMessageIDs) {
    ...
    EXPECT_EQ(GetResponseMsgID(MSG_GET_JOB_RESULT_REQUEST), MSG_GET_JOB_RESULT_RESPONSE);
}
...
TEST_F(ProtocolTest, InvokerServiceMessageIDs) {
    EXPECT_EQ(GetResponseMsgID(MSG_INVOKE_REQUEST), MSG_INVOKE_RESPONSE);
    EXPECT_EQ(GetResponseMsgID(MSG_START_JOB_REQUEST), MSG_START_JOB_RESPONSE);
    EXPECT_EQ(GetResponseMsgID(MSG_CANCEL_JOB_REQUEST), MSG_CANCEL_JOB_RESPONSE);
}
...
TEST_F(ProtocolTest, MsgIDStringAllKnown) {
    ...
    std::vector<uint32_t> known_msgs = {
        ...
        MSG_GET_JOB_RESULT_REQUEST, MSG_GET_JOB_RESULT_RESPONSE,
        MSG_INVOKE_REQUEST, MSG_INVOKE_RESPONSE,
        MSG_START_JOB_REQUEST, MSG_START_JOB_RESPONSE,
        MSG_STREAM_JOB_REQUEST, MSG_JOB_EVENT,
        MSG_CANCEL_JOB_REQUEST, MSG_CANCEL_JOB_RESPONSE,
