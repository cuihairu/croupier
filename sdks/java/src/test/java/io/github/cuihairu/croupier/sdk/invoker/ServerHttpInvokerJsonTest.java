...
class ServerHttpInvokerJsonTest {
    ...
    void parsePrimitives() {
        ...
        assertEquals(0.0, (Double) Json.parse("0"), 1e-9);
        assertEquals(-7.0, (Double) Json.parse("-7"), 1e-9);
        assertEquals(9223372036854775807.0, (Double) Json.parse("9223372036854775807"), 1e6);
        assertEquals(3.14, (Double) Json.parse("3.14"), 1e-9);
        assertEquals(1000.0, (Double) Json.parse("1e3"), 1e-9);
        assertEquals(0.0015, (Double) Json.parse("1.5E-3"), 1e-9);
        assertEquals(100.0, (Double) Json.parse("1.0e+2"), 1e-9);
        assertEquals("hi", Json.parse("\"hi\""));
        assertEquals(" spaced ", Json.parse("  \" spaced \"  "));
    }
    ...
    @SuppressWarnings("unchecked")
    void parseNested() {
        Object value = Json.parse("{\"a\":[1,{\"b\":[true,null]},\"x\"],\"c\":{}}");
        ...
        assertEquals(List.of(1.0, 2.0, 3.0), Json.parse("[1,2,3]"));
    }
    ...
    void stringifyValues() {
        ...
        ordered.put("b", "v");
        assertEquals("{\"a\":[1,2],\"b\":\"v\"}", Json.stringify(ordered));
        // Recorded bug: parsed integers are Doubles, so re-serializing turns 42 into 42.0.
        assertEquals("42.0", Json.stringify(Json.parse("42")));
    ...
    void roundTrip() {
        ...
        assertEquals(parsed, Json.parse(Json.stringify(parsed)));
    ...
    private static void assertParseError(String source) {
        IllegalArgumentException error = assertThrows(IllegalArgumentException.class,
            () -> Json.parse(source), "expected parse failure for: " + source);
