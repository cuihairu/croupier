...
当前 Croupier Go SDK 正处于早期开发阶段，以下版本获得安全更新支持：
...
使用 Croupier Go SDK 时，建议遵循以下安全实践：

- 始终使用最新稳定版本
- 定期运行 `go mod tidy` 和 `govulncheck` 检查依赖漏洞
- 在生产环境中启用 TLS/mTLS 加密通信
- 使用环境变量或 Vault 等工具管理敏感配置
- 避免在日志中输出敏感信息
