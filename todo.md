...
- 本轮（2026-08-07）新增前端验证：`cd "web" && pnpm exec eslint "src/pages/OpenAPISources/index.tsx" "src/components/ProposalInbox/index.tsx" "src/services/api/openapi.ts" "src/components/SchemaFormRenderer/__tests__/game-schema.test.tsx" "src/components/SchemaFormRenderer/__tests__/presentation-adapter.test.tsx"` 通过；`cd "web" && pnpm exec tsc --noEmit` 通过。期间清理了 SchemaFormRenderer 测试中的旧 `schema/onSubmit` prop 和 `@jest/globals` 引入，测试只使用当前 `FormPresentationSpec`/`spec` 契约。
