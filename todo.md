...
- 本轮（2026-08-07）新增前端事实：Proposal Inbox 的 `ready/basic` 现在支持预览、接受后自动进入 Page Studio，以及直接发布后触发左侧 Console 动态菜单刷新并直达刚发布的运行页；`needs_review` 不再把所有带 `resourceKey` 的提案一律送入 Resource Catalog，而是仅资源页进入 Resource Catalog，独立 Operation/Task/Report 走页面处理路径。以上仅完成目标 ESLint 与后端链路验证，尚未完成浏览器 E2E。
...
- 未通过或未验收的检查：全量 Playwright E2E、docs build、SDK parity、部署验证、全量 `go test ./...`。其中 `go test ./internal/...` 在当前受限环境下会卡在 `internal/agent` 的 TCP listener 测试（`socket: operation not permitted`），不能作为完成证据。
...
- 2026-08-07 新增事实：发布、取消发布和重新发布现在都会触发 ProLayout 侧 `console-menu:refresh` 事件，使左侧运行控制台动态菜单立即按最新 PublishedPageSpec 重取；Proposal Inbox 的直接发布确认框也会优先直达刚发布页面的 `/console/:categoryKey/:pageKey`，而不是只停留在管理页提示成功。以上仍缺真实浏览器 E2E，不得当作 P6 完成证据。
