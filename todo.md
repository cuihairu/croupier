...
- 以下检查的历史“曾通过”记录不作为交接依据；必须重新执行并记录命令与结果后才可作为证据：Dashboard PageSpec guard、目标后端包测试。
- 本轮（2026-08-07）已重新执行并通过的检查：`bash "scripts/dashboard_vnext_guard.sh"`；`GOCACHE="/tmp/croupier-go-build" GOMODCACHE="/tmp/croupier-go-mod" go test ./internal/platform/registry ./internal/model ./internal/api/openapi ./internal/api/resource ./internal/api/console ./internal/api/approval ./internal/api/page ./internal/logic/function ./internal/service -count=1`；`GOCACHE="/tmp/croupier-go-build" GOMODCACHE="/tmp/croupier-go-mod" go test ./internal/platform/registry ./internal/server -run 'TestStore_UpsertAgent|TestStore_UpsertAgentUsesScopedContextForContractRebuild|TestStore_UpsertAgentFailsWhenContractRebuildFails|TestControlService_HandleRegisterRequest|TestValidateAndNormalizeFunctions' -count=1`；`GOCACHE="/tmp/croupier-go-build" GOMODCACHE="/tmp/croupier-go-mod" go test ./cmd/server -run 'TestWireDashboardRegistrationPipelineGeneratesProposal|TestDashboardRegistrationProposalPublishesToConsoleAndExecutes|TestOpenAPIBindingProposalPublishesToConsoleAndExecutes' -count=1`；`cd "web" && pnpm exec eslint "src/components/ProposalInbox/index.tsx" "src/app.tsx" "src/utils/consoleMenu.ts" "src/pages/PageStudio/index.tsx" "src/pages/Ops/Nodes/index.tsx"`；`cd "web" && pnpm exec tsc --noEmit --pretty false`；`cd "web" && pnpm exec jest "tests/consoleMenu.test.ts" --runInBand`；`cd "web" && pnpm exec playwright test --reporter=line` 通过 42/42；`rg "@formily|components/formily|Formily|formily|generateFormily|validateFormily" web/src web/package.json` 无命中。
- 本轮（2026-08-07）新增已验证核心后端链路：`GOCACHE="/tmp/croupier-go-build" GOMODCACHE="/tmp/croupier-go-mod" go test ./cmd/server -run 'TestDashboardRegistrationProposalPublishesToConsoleAndExecutes|TestOpenAPIBindingProposalPublishesToConsoleAndExecutes' -count=1`。覆盖 SDK 注册 `player.ban -> operation:player.ban -> accept/publish -> Console menu/page -> binding execute`，以及 OpenAPI Source `/players` + Provider binding `player.list -> resource:players -> accept/publish -> Console menu/page -> binding execute`；Console execute payload 由 PublishedPageSpec selector 构造，浏览器不传 functionId/game/env/target。
- 本轮（2026-08-07）新增前端事实：Proposal Inbox 的 `ready/basic` 现在支持预览、接受后自动进入 Page Studio，以及直接发布后触发左侧 Console 动态菜单刷新并直达刚发布的运行页；`needs_review` 不再把所有带 `resourceKey` 的提案一律送入 Resource Catalog，而是仅资源页进入 Resource Catalog，独立 Operation/Task/Report 走页面处理路径。以上已通过目标 ESLint、全量 `tsc` 与真实浏览器 E2E（42/42）。
- 本轮（2026-08-07）新增注册边界事实：SDK 注册若在 `FunctionContract/CapabilitySemantics/PageProposal` 重建阶段失败，现在会直接返回错误，且不会把该 Agent 写入内存 registry，避免“注册成功但默认页面未生成”的假成功。
- 未通过或未验收的检查：docs build、SDK parity、部署验证、全量 `go test ./...`。其中 `go test ./internal/...` 在当前受限环境下会卡在 `internal/agent` 的 TCP listener 测试（`socket: operation not permitted`），不能作为完成证据。
...
- 2026-08-07 新增事实：SDK 注册与 OpenAPI Provider binding 的最短后端产品链路已通过跨模块目标测试。SDK 路径覆盖 `player.ban` 注册、生成 `operation:player.ban`、直接发布、Console 菜单与受控执行；OpenAPI 路径覆盖 Source `/players`、绑定已注册 provider `player.list`、返回 `resource:players` Proposal 摘要、直接发布、Console 菜单与受控执行。OpenAPI path 默认不做单复数转换，`/players` 的 inferred `resourceKey` 是 `players`；业务若要求 `player`，必须显式提供 `x-resource: player`。
...
- 2026-08-07 新增事实：当 ResourcePage 因 selector/schema/identity 不可安全物化而无法生成时，未被资源页吸收的 CRUD 函数现在会回退为独立 `OperationPage Proposal`；反之已被 ResourcePage 吸收的 CRUD/action 不再重复生成独立 Proposal，避免“没有默认页面”或“一个函数出现两份菜单”。
...
- OpenAPI Source 页面已接入当前 Proposal 路径：Provider binding 成功后后端返回生成的 Proposal 摘要，前端提示并跳转到 Proposal Inbox；Proposal Inbox 支持 `proposalKey` 查询参数自动打开预览。Source/Binding 仍不写 UI、菜单、标题或分类 labels。
