...
## U10. 区块级条件显示（P2，可选）✅（visibleWhen，2026-09 交付）
...
## U11. 组件模板更新提醒（P2，可选）✅（digest 快照比对，2026-09 交付）
...
## S2. CapabilitySemantics 感知的 identity 字段 row 源推断（P2，后续增强）✅（2026-09 交付）
...
# V 阶段：编辑器组合入口发现性（2026-09-12，产品分析产出）

背景：组合页编辑器是用户体验核心，但「创建新组件/组合」能力入口全部隐蔽——保存为组件按钮仅多选后才渲染（`index.tsx` multiIds.size > 0 条件）、多选机制（Shift+点击）零提示、组件库面板有模板时无创建入口、模板卡片无结构预览、无右键保存。能力存在但不可见等于不存在（对比 Retool/lowcode-engine 的区块面板均常驻创建入口+缩略图预览）。

## V1. 保存为组件四项入口发现性（P0） ✅

**目标**：用户无需文档即可发现「画布组合 → 保存为可复用组件」的完整闭环。

**改动点**：

- [x] 顶栏「保存为组件」按钮常驻：未多选时禁用 + Tooltip 教学（「Shift+点击 多选画布节点后可保存」）；多选后启用并显示计数
- [x] 组件库面板头部加「从画布选中创建」入口（回调由编辑器接线：无选中时 message 提示先多选，有选中直接弹保存弹窗）
- [x] 模板卡片加结构缩略图（新组件 TemplateThumb：节点树→轻量线框图——表格=表头+行线、表单=标签/输入行、按钮=圆角块、弹窗=图标标记，纯 div 渲染不实例化真实组件）；组件库与「从模板开始」引导两处卡片共用
- [x] 节点右键菜单加「保存为组件」：多选集合含该节点时保存整个集合，否则保存单节点子树；`saveSelectionAsComponent` 重构为接受显式 ids 参数（不经 setState 异步竞态）
- [x] i18n：新增 5 枚 key 同步 zh-CN/en-US 语言包
- [x] 测试：TemplateThumb 线框结构（表格/表单/按钮/弹窗形态）、组件库创建入口回调与缺失不渲染、右键菜单保存回调（单节点/多选集合两分支）、常驻按钮禁用态+tooltip+启用流转（全页集成渲染）
- [x] 文档：composite-editor-v3.md「入口分工」段落同步四入口
