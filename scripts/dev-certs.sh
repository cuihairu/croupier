...
cat > "$OUT/server-openssl.cnf" <<EOF
[req]
...
openssl req -new -key "$OUT/server.key" -subj "/CN=croupier-server" -out "$OUT/server.csr" -config "$OUT/server-openssl.cnf" >/dev/null 2>&1
openssl x509 -req -in "$OUT/server.csr" -CA "$OUT/ca.crt" -CAkey "$OUT/ca.key" -CAcreateserial \
  -out "$OUT/server.crt" -days 365 -sha256 -extensions v3_req -extfile "$OUT/server-openssl.cnf" >/dev/null 2>&1
