...
| `StatisticCard` / `ProCard`                                                      | 统计卡/卡片栅格布局                                     | 2026-09 已评估并迁移 5 文件：Overview(12 卡)/DetailTabs(4)/History(6)/Cluster(3) 的手写 `Card>Statistic` 嵌套 + Realtime/StatCard 本地抽象（chart slot 替换）                                                    | ✅ 卡墙形态（Card-per-Statistic）一律用 `StatisticCard`。6 个保留文件是有意的内联统计条/紧凑 facet（Invocations/Levels/SdkDistribution/LBMonitor/Profile）或契约渲染面（PageSchemaRenderer，需线上视觉回归后单独立项）。已知视觉边界：pro Statistic 标题为正文色（antd 默认次要灰）、pro Card loading 为静态灰块（antd 为骨架屏）；数值样式用 `styles.content`（antd 6 `valueStyle` 已废弃） |
...
| `StepsForm` / `LoginForm`                                                        | 分步表单/登录表单                                       | 2026-09 已评估：`LoginForm` 已采用（`User/Login`，含 MFA 动态字段/OIDC SSO/LDAP 提示）；`StepsForm` 无存量分步表单可迁移（全库唯一 antd `Steps` 使用是 PageWorkflowGuide 纯展示引导，current={-1} 非交互无表单） | ✅ 登录类页面一律 `LoginForm`；出现分步表单需求时禁止手写 `Steps`+`Form` 联动，用 `StepsForm`                                                                                                                                                                                                                                                                                                |
