...
### ✅ T4.4 回读编辑（原 V1.1 预留，已于 V3.1 提前完成）

- 改动：decompileToTree（spec → 编辑树：dialog→弹窗容器、引用 section key→节点 id、
  顶部按钮还原为独立按钮节点）；编辑器 `?pageKey=` 自动载入（提案 composite--key →
  裸 key → draft 三数据源 fallback）；页面管理列表 composite「编辑」入口
- 验收：decompile 测试 3 用例（结构还原/回读→再编译 round-trip 等价/破损引用降级）
- 依赖：T4.1 ｜ 实际完成于 V3.1/V3.2 批次
...
## V3.1 边界清零（2026-08-30 追加批次，评审要求「全部完成」）

| 项           | 内容                                                                           |
| ------------ | ------------------------------------------------------------------------------ |
| 多实例       | 后端 CompositeSectionRequest.Key；重复函数不再丢弃，key 唯一校验（fid/fid-2…） |
| 换绑         | 属性面板函数全量下拉，换绑重 scaffold                                          |
| 向导移除     | 强制向导删除（参考产品无此模式），空画布=拖拽占位                              |
| 弹窗多组件   | Section.Group 分组：同 group 渲染进同一弹窗（表单+字段卡+表格混排）            |
| 动作链       | CompositeActionStep（Chain）：动作后按序执行；编辑器链 UI；发布执行            |
| 容器子级     | 点选/删除/右键同级移动；拖入容器进 children                                    |
| 顶部按钮回读 | 反编译还原独立按钮节点（含链），round-trip 等价                                |
| 回读多数据源 | 提案 composite--key → 裸 key → draft fallback                                  |
| 多选         | 点选累积+批量删除                                                              |
