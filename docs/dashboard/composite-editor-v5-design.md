...
> 状态：**T5.1–T5.6 已实现**（编辑器/渲染器全链 + 单测覆盖）；**T5.7 线上端到端验收待执行**
...
> 实现落点（与设计一致，补充两处落地细节）：
>
> - 变量名存储于 `props.sectionKey`（== 发布 spec 区块 key，D1）；拖入/模板实例化经 `assignVarNames` 自动命名与去重。
> - page_state 快照为**双形态**：扁平值兼容遗留 `/字段` 路径 + `values` 包装供 <code v-pre>{{var.values.x}}</code>；
>   表格选中行与函数输出以 merge 模式并入（`selectedRow`/`data` 分支），服务端 inputAssignment 路径求值不受影响。
> - 行操作 wire 契约：编辑器产出顶层 `rowActions`（POST 请求），服务端落库于 `table.rowActions`，回读按后者还原。
