...
> **状态**：Active -- 本文是根目录 `todo.md` 任务 `A-002` 的产物，是 `H-001`~`H-005` 物理删除任务的唯一逐项依据。每个旧文件、路由、DTO、API、表/列都必须在本清单中登记替代任务、owner、删除前置条件（E2E）与删除任务。

## 使用规则

- 「废弃」不等于删除完成。只有**物理删除 + guard 防回流**同时成立，清单项才能标记为「已删除」。
- 不得删除生产数据。`H-005` 涉及生产数据删除，必须取得单独明确确认，并先完成备份校验。
- 执行 `H-*` 任务时逐项引用本清单的条目编号；删除完成后由执行者更新「状态」列，审核者复跑 Verify 后方可在 `todo.md` 勾选。
...
| H-002-15 | `web/docs/` 下 8 个旧 workspace 文档（DEMO/WORKSPACE_* 等） | 文档 | 已删除 | `G-*` | H-002 | 同上 | `H-002` | model guard `removed_paths` + web/docs 符号扫描 |
...
| # | 清单项 | 类型 | 状态 | 替代任务 | Owner | 删除前置条件（E2E） | 删除任务 | 防回流证据 |
|---|--------|------|------|----------|-------|--------------------|----------|-----------|
| H-005-1 | 生产库中可能残留的历史页面表（如旧 `function_ui*` 版本表） | 表 | 待复核 | `D-001`~`D-006` | H-005 | 备份校验 + 迁移测试 + 全量 E2E + deployment dry-run + 单独明确确认 | `H-005` | 代码库已无模型定义；H-005 交付版本化清理函数 |
| H-005-2 | 现有表上的旧列（注册侧 UI 字段对应存储列，若存在） | 列 | 待复核 | `B-001` | H-005 | 同上 | `H-005` | H-004 完成后逐表核查 |
| H-005-3 | 旧页面/旧 UI 历史数据行 | 数据 | 待复核 | `D-006` | H-005 | 同上（生产数据删除必须单独确认） | `H-005` | 备份校验记录 |
