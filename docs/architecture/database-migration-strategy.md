...
4. ✅ CI 迁移测试覆盖四方言（`ci.yml` 的 `migrate-matrix` job：真实 MySQL 8.4 + Postgres 16 + SQL Server 2022 services，含并发首开锁验证；SQLite 由单测覆盖）与 Atlas 校验（社区版 `atlas migrate validate` 校验 `atlas.sum` 与文件顺序；注意 `atlas migrate lint` 自 v0.38 起 Pro 付费，规则化 lint 暂不可用）。

   **SQL Server JSON 列**：`internal/model.JSON`（jsontype.go）是 `datatypes.JSON` 的方言感知替代——语义逐方法对齐（Value/Scan/MarshalJSON/GormValue 的 mysql CAST），唯一差异是 `GormDBDataType` 默认分支返回 `nvarchar(max)`（datatypes.JSON 对 sqlserver 返回空，gorm 驱动回落字面量 `json` 而 SQL Server 无此类型）。`datatypes.JSONMap` 原生已含 sqlserver 分支，模型继续使用。会话锁用 `sp_getapplock`（注意 go-mssqldb 不重写多语句批次内的 `?` 占位符，资源名内联）。
