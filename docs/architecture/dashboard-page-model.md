...
| `composite` | `CompositePageSpec`：`sections[]`（每区块绑定一个函数；`display` inline/dialog、`group` 弹窗分组、`rowActions`/`toolbar` 按钮动作含 `chain` 动作链、`onSuccessRefresh`、`refreshOn` page_state 联动），另有 `static` 常量表单（不绑定函数，值进 page_state；sections 顺序=请求输入顺序，static 与函数区块按输入位置交错，不重排到末尾） |
