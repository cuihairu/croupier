...
| P1     | ~~失败策略 + transform 扩展（rename/default/format）~~ ✅ 2026-09（todo U8/U9） | 数据流健壮性：transform rename/default；refreshOn 级联 cascadePolicy（pause/clear/keep）                                                                                             |
| P1     | ~~跨模板联动断链提示（实例化悬空引用静默清理）~~ ✅ 2026-09（todo U7）          | instantiateTemplateDetailed 检出四类悬空引用（动作目标/联动依赖/参数映射来源/行操作弹窗），拖入后 Modal 提示 + 画布区块快捷重连（reconnectTemplateRefs）；未重连项保存时编译警告兜底 |
