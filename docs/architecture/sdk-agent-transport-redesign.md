...
- Java / C# / JS / Python 对 NNG 绑定质量和本地运行时分发高度敏感。
- SDK 作为被业务程序嵌入的依赖时，引入原生 NNG 运行时会增加接入复杂度、产物体积和 CI 不稳定性。
...
本设计的默认路径不再依赖 SDK 侧 `NNG`。如后续出于本地高性能场景保留 `NNG`，也只能作为非默认、明确 opt-in 的兼容选项，不能继续作为 SDK 默认依赖。
...
对于“把 SDK 嵌进其他用户程序”的场景，SDK 侧继续依赖 NNG 的收益已经明显低于成本。  
