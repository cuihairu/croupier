...
> **已知边界**：挂载操作当前通过 API 完成，管理界面的挂载入口尚未提供。

对已发布页面调用挂载接口（要求 `pages:edit` 或 `admin:all` 权限）：
...
解除挂载（页面从控制台消失，但发布状态保留、直达 URL 仍可访问）：
...
**Q：挂载错了菜单怎么改？**
直接再 PUT 一次目标 `menuId` 即可，即时生效，无需解除再挂。
